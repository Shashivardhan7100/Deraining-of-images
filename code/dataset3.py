...
def make_dataset(root, is_train):
    if is_train:
        ...
        input = open(os.path.join(root, 'train_input.txt'))
        ground_t = open(os.path.join(root, 'train_gt.txt'))
        depth_t = open(os.path.join(root, 'train_depth.txt'))
        image = [(os.path.join(root, 'train\input', img_name.strip('\n'))) for img_name in
                 input]
        gt = [(os.path.join(root, 'train\labels', img_name.strip('\n'))) for img_name in
                 ground_t]
        depth = [(os.path.join(root, 'train\depth', img_name.strip('\n'))) for img_name in
              depth_t]
        ...
        input = open(os.path.join(root, 'test_input.txt'))
        ground_t = open(os.path.join(root, 'test_gt.txt'))
        depth_t = open(os.path.join(root, 'test_depth.txt'))

        image = [(os.path.join(root, 'test\input', img_name.strip('\n'))) for img_name in
                 input]
        gt = [(os.path.join(root, 'test\labels', img_name.strip('\n'))) for img_name in
              ground_t]
        depth = [(os.path.join(root, 'test\depth', img_name.strip('\n'))) for img_name in
                 depth_t]
